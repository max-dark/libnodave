...
#ifdef HAVE_SELECT
int CALL_SPEC stdwrite(daveInterface * di, char * buffer, int length) {
    if (daveDebug & daveDebugByte)
        _daveDump("I send", (uc*)buffer, length);
    return write(di->fd.wfd, buffer, length);
}

int CALL_SPEC stdread(daveInterface * di, char * buffer, int length) {
    fd_set FDS;
    struct timeval t;
    int i;
...
int CALL_SPEC stdread(daveInterface * di,
    char * buffer,
    int length) {
    unsigned long i;
...
*/
daveInterface * CALL_SPEC daveNewInterface(_daveOSserialType nfd, const char * nname, int localMPI, int protocol, int speed) {
...
daveInterface * CALL_SPEC davePascalNewInterface(_daveOSserialType* nfd, char * nname, int localMPI, int protocol, int speed) {
#ifdef DEBUG_CALLS
...
void CALL_SPEC daveSetDebug(int nDebug) {
#ifdef DEBUG_CALLS
    LOG2("daveSetDebug(%d)\n", nDebug);
    FLUSH;
#endif
...
/**
    C# interoperability:
**/
void CALL_SPEC daveSetTimeout(daveInterface * di, int tmo) {
#ifdef DEBUG_CALLS
...
int CALL_SPEC daveGetTimeout(daveInterface * di) {
#ifdef DEBUG_CALLS
...
char * CALL_SPEC daveGetName(daveInterface * di) {
#ifdef DEBUG_CALLS
...
int CALL_SPEC daveGetMPIAdr(daveConnection * dc) {
#ifdef DEBUG_CALLS
...
int CALL_SPEC daveGetAnswLen(daveConnection * dc) {
#ifdef DEBUG_CALLS
...
int CALL_SPEC daveGetMaxPDULen(daveConnection * dc) {
#ifdef DEBUG_CALLS
...
*/
void CALL_SPEC _daveInitPDUheader(PDU * p, int type) {
...
/*
    ...
    needs valid header
*/
void CALL_SPEC _daveAddParam(PDU * p, uc * param, us len) {
#ifdef DEBUG_CALLS
...
*/
void CALL_SPEC _daveAddData(PDU * p, void * data, int len) {
#ifdef DEBUG_CALLS
...
*/
void CALL_SPEC _daveAddValue(PDU * p, void * data, int len) {
    us dCount;
    uc * dtype;
#ifdef DEBUG_CALLS
...
*/
void CALL_SPEC _daveAddUserData(PDU * p, uc * da, int len) {
    uc udh[] = { 0xff,9,0,0 };
    if (p->dlen == 0) {
        if (daveDebug & daveDebugPDU)
...
void CALL_SPEC davePrepareReadRequest(daveConnection * dc, PDU *p) {
...
void CALL_SPEC davePrepareWriteRequest(daveConnection * dc, PDU *p) {
...
void CALL_SPEC daveAddToReadRequest(PDU *p, int area, int DBnum, int start, int byteCount, int isBit) {
    uc pa[] = {
    0x12, 0x0a, 0x10,
...
void CALL_SPEC daveAddVarToReadRequest(PDU *p, int area, int DBnum, int start, int byteCount) {
#ifdef DEBUG_CALLS
...
void CALL_SPEC daveAddBitVarToReadRequest(PDU *p, int area, int DBnum, int start, int byteCount) {
    daveAddToReadRequest(p, area, DBnum, start, byteCount, 1);
}

void CALL_SPEC daveAddToWriteRequest(PDU *p, int area, int DBnum, int start, int byteCount,
    void * buffer,
    uc * da,
    int dasize,
    uc * pa,
...
void CALL_SPEC daveAddVarToWriteRequest(PDU *p, int area, int DBnum, int start, int byteCount, void * buffer) {
    uc da[] = { 0,4,0,0, };
    uc pa[] = {
        0x12, 0x0a, 0x10,
...
void CALL_SPEC daveAddBitVarToWriteRequest(PDU *p, int area, int DBnum, int start, int byteCount, void * buffer) {
    uc da[] = { 0,3,0,0, };
    uc pa[] = {
        0x12, 0x0a, 0x10,
        0x01,		/* single bit */
...
*/
int CALL_SPEC daveGetPDUerror(PDU * p) {
#ifdef DEBUG_CALLS
...
*/
int CALL_SPEC _daveSetupReceivedPDU(daveConnection * dc, PDU * p) {
...
int CALL_SPEC _daveTestResultData(PDU * p) {
    int res; /*=daveResCannotEvaluatePDU;*/
...
int CALL_SPEC _daveTestWriteResult(PDU * p) {
    int res;/* =daveResCannotEvaluatePDU; */
...
*/
int CALL_SPEC _daveMemcmp(us * a, uc *b, size_t len) {
    unsigned int i;
    us * a1 = (us*)a;
    uc * b1 = (uc*)b;
    for (i = 0; i < len; i++) {
...
/*
    Hex dump:
*/
void CALL_SPEC _daveDump(const char * name, void*b, int len) {//void CALL_SPEC _daveDump(char * name,uc*b,int len) {
    int j;
    LOG2("%s:                             ", name);
...
/*
    Hex dump PDU:
*/
void CALL_SPEC _daveDumpPDU(PDU * p) {
    int i, dl;
    uc * pd;
...
/*
    name Objects:
*/
const char * CALL_SPEC daveBlockName(uc bn) {
    //#ifdef DEBUG_CALLS
...
const char * CALL_SPEC daveAreaName(uc n) {
#ifdef DEBUG_CALLS
...
*/
void CALL_SPEC _daveConstructUpload(PDU *p, char blockType, int blockNr) {
    uc pa[] = { 0x1d,
...
void CALL_SPEC _daveConstructDoUpload(PDU * p, int uploadID) {
...
void CALL_SPEC _daveConstructEndUpload(PDU * p, int uploadID) {
...
int CALL_SPEC daveStop(daveConnection * dc) {
    int res;
    PDU p, p2;
#ifdef DEBUG_CALLS
    LOG2("daveStop(dc:%p)\n", dc);
...
int CALL_SPEC daveStart(daveConnection*dc) {
    int res;
    PDU p, p2;
#ifdef DEBUG_CALLS
    LOG2("daveStart(dc:%p)\n", dc);
...
int CALL_SPEC daveCopyRAMtoROM(daveConnection * dc) {
    int res;
    PDU p, p2;
#ifdef DEBUG_CALLS
...
*/
int CALL_SPEC daveBuildAndSendPDU(daveConnection * dc, PDU*p2, uc *pa, int psize, uc *ud, int usize) {
    int res;
    PDU p;
...
int CALL_SPEC daveListBlocksOfType(daveConnection * dc, uc type, daveBlockEntry * buf) {
    int res, i, len;
    PDU p2;
    uc * buffer = (uc*)buf;
...
*/
int CALL_SPEC daveGetOrderCode(daveConnection * dc, char * buf) {
    int res = 0;
    PDU p2;
...
int CALL_SPEC daveReadSZL(daveConnection * dc, int ID, int index, void * buffer, int buflen) {
    int res, len, cpylen;
    int pa7;
    //    int pa6;
    PDU p2;
...
int CALL_SPEC daveGetBlockInfo(daveConnection * dc, daveBlockInfo *dbi, uc type, int number) {
    int res;
...
int CALL_SPEC daveListBlocks(daveConnection * dc, daveBlockTypeEntry * buf) {
    int res, i;
    PDU p2;
...
int CALL_SPEC daveReadManyBytes(daveConnection * dc, int area, int DBnum, int start, int len, void * buffer) {
    int res, pos, readLen;
    uc * pbuf;
    pos = 0;
...
*/
int CALL_SPEC daveReadBytes(daveConnection * dc, int area, int DBnum, int start, int len, void * buffer) {
    PDU p1, p2;
    int res;
#ifdef DEBUG_CALLS
...
/*
    ...
    as you do not send further requests.
*/
int CALL_SPEC daveReadBits(daveConnection * dc, int area, int DBnum, int start, int len, void * buffer) {
    PDU p1, p2;
    int res;
#ifdef DEBUG_CALLS
...
*/
int CALL_SPEC daveExecReadRequest(daveConnection * dc, PDU *p, daveResultSet* rl) {
    PDU p2;
    uc * q;
    daveResult * cr, *c2;
    int res, i, len, rlen;
...
*/
int CALL_SPEC daveExecWriteRequest(daveConnection * dc, PDU *p, daveResultSet* rl) {
    PDU p2;
    uc * q;
    daveResult * cr, *c2;
    int res, i;
...
int CALL_SPEC daveUseResult(daveConnection * dc, daveResultSet * rl, int n) {
    daveResult * dr;
#ifdef DEBUG_CALLS
...
void CALL_SPEC daveFreeResults(daveResultSet * rl) {
    daveResult * r;
    int i;
#ifdef DEBUG_CALLS
...
int CALL_SPEC daveGetErrorOfResult(daveResultSet *rs, int number) {
...
*/
daveConnection * CALL_SPEC daveNewConnection(daveInterface * di, int MPI, int rack, int slot) {
...
void CALL_SPEC daveSetRoutingDestination(daveConnection * dc, int subnet1, int subnet3, int adrsize, uc* plcadr) {
...
void CALL_SPEC daveSetCommunicationType(daveConnection * dc, int communicationType) {
    dc->communicationType = communicationType;
}


int CALL_SPEC daveWriteManyBytes(daveConnection * dc, int area, int DBnum, int start, int len, void * buffer) {
...
int CALL_SPEC daveWriteBytes(daveConnection * dc, int area, int DB, int start, int len, void * buffer) {
    PDU p1, p2;
    int res;
    if (dc->iface->protocol == daveProtoAS511) {
...
int CALL_SPEC daveWriteBits(daveConnection * dc, int area, int DB, int start, int len, void * buffer) {
    PDU p1, p2;
    int res;
    p1.header = dc->msgOut + dc->PDUstartO;
    davePrepareWriteRequest(dc, &p1);
...
*/
int CALL_SPEC daveSetBit(daveConnection * dc, int area, int DB, int byteAdr, int bitAdr) {
    int a = 1;
...
*/
int CALL_SPEC daveClrBit(daveConnection * dc, int area, int DB, int byteAdr, int bitAdr) {
    int a = 0;
    return daveWriteBits(dc, area, DB, 8 * byteAdr + bitAdr, 1, &a);
}


int CALL_SPEC initUpload(daveConnection * dc, char blockType, int blockNr, int * uploadID) {
    PDU p1, p2;
    int res;
...
int CALL_SPEC doUpload(daveConnection*dc, int * more, uc**buffer, int*len, int uploadID) {
    PDU p1, p2;
    int res, netLen;
    p1.header = dc->msgOut + dc->PDUstartO;
...
int CALL_SPEC endUpload(daveConnection*dc, int uploadID) {
    PDU p1, p2;
    int res;
...
*/
const char * CALL_SPEC daveStrerror(int code) {
    switch (code) {
...
*/
void CALL_SPEC daveStringCopy(char * intString, char * extString) {
...
*/
float CALL_SPEC daveGetKGAt(daveConnection * dc, int pos) {
    char kgExponent;
    int sign;
    union {
        uc b[4];
...
float CALL_SPEC daveGetKG(daveConnection * dc) {
    float f;
    f = daveGetKGAt(dc, (int)(dc->resultPointer - dc->_resultPointer));
    dc->resultPointer += 4;
    return f;
}

float CALL_SPEC daveGetFloat(daveConnection * dc) {
    union {
        float a;
        uc b[4];
    } f;
...
float CALL_SPEC daveGetFloatAt(daveConnection * dc, int pos) {
    union {
        float a;
        uc b[4];
    } f;
...
float CALL_SPEC toPLCfloat(float ff) {
#ifdef DAVE_LITTLE_ENDIAN
    union {
        float a;
        uc b[4];
...
int CALL_SPEC daveToPLCfloat(float ff) {
    union {
        float a;
        uc b[4];
        int c;
...
int CALL_SPEC daveToKG(float ff) {
    union {
        uc b[4];
        int c;
    } f, f2;
...
i16_t CALL_SPEC daveSwapIed_16(i16_t ff) {
#ifdef DAVE_LITTLE_ENDIAN
    union {
        short a;
        uc b[2];
...
i32_t CALL_SPEC daveSwapIed_32(i32_t ff) {
#ifdef DAVE_LITTLE_ENDIAN
    union {
        int a;
        uc b[4];
...
*/
float CALL_SPEC daveGetSeconds(daveConnection * dc) {
    uc b[2], a;
    float f;
    ...
    a = ((b[1] & 0xf0) >> 4);
    switch (a) {
    case 0: f *= 0.01f; break;
    case 1: f *= 0.1f; break;
    case 3: f *= 10.0f; break;
    }
    return (f);
}
/*
...
*/
float CALL_SPEC daveGetSecondsAt(daveConnection * dc, int pos) {
    float f;
    uc b[2], a;
    uc* p = (uc*)dc->_resultPointer;
    p += pos;
...
*/
int CALL_SPEC daveGetCounterValue(daveConnection * dc) {
    uc b[2];
    int f;
    b[1] = *(dc->resultPointer)++;
    b[0] = *(dc->resultPointer)++;
...
*/
int CALL_SPEC daveGetCounterValueAt(daveConnection * dc, int pos) {
    int f;
    uc b[2];
    uc* p = (uc*)dc->_resultPointer;
    p += pos;
...
int CALL_SPEC _daveReturnOkDummy(daveInterface * di) {
    return 0;
}

int CALL_SPEC _daveReturnOkDummy2(daveConnection * dc) {
    return 0;
}

int CALL_SPEC _daveListReachablePartnersDummy(daveInterface * di, char * buf) {
    return 0;
}
...
void CALL_SPEC _daveSendSingle(daveInterface * di,	/* serial interface */
...
int CALL_SPEC _daveReadSingle(daveInterface * di) {
    char res;
    int i;
...
int CALL_SPEC _daveReadMPI(daveInterface * di, uc *b) {
...
int CALL_SPEC _daveReadMPI2(daveInterface * di, uc *b) {
    uc b6;
    uc b2[daveMaxRawLen];
...
int CALL_SPEC _daveGetAck(daveConnection * dc) {
    int res;
...
*/
int CALL_SPEC _daveReadChars2(daveInterface * di,	/* serial interface */
    uc *b, 		/* a buffer */
    int max		/* limit */
)
{
...
*/
int CALL_SPEC _daveSendWithCRC(daveInterface * di, /* serial interface */
...
/*
    ...
    and adding DLE,ETX and bcc.
*/
int CALL_SPEC _daveSendWithPrefix(daveConnection * dc, uc *b, int size)
{
    uc target[daveMaxRawLen];
...
int CALL_SPEC _daveSendWithPrefix2(daveConnection * dc, int size)
{
    uc fix[] = { 04,0x80,0x80,0x0C,0x03,0x14 };
    uc fix2[] = { 0x00, 0x0C, 0x03, 0x03 };
...
*/
int CALL_SPEC _daveSendAck(daveConnection * dc, int nr)
{
    uc m[3];
    if (daveDebug & daveDebugPacket)
...
*/
int CALL_SPEC _daveIncMessageNumber(daveConnection * dc) {
...
*/
int CALL_SPEC _daveSendDialog2(daveConnection * dc, int size)
{
    int a;
...
int CALL_SPEC _daveGetResponseMPI(daveConnection *dc) {
    int res;
...
*/
int CALL_SPEC _daveSendMessageMPI(daveConnection * dc, PDU * p) {
    if (daveDebug & daveDebugExchange) {
...
int CALL_SPEC _daveExchangeMPI(daveConnection * dc, PDU * p) {
...
*/
int CALL_SPEC _daveInitStep(daveInterface * di, int nr, uc *fix, int len, char * caller) {
    _daveSendSingle(di, STX);
...
*/
int CALL_SPEC _daveInitAdapterMPI2(daveInterface * di)  /* serial interface */
{
    uc b3[] = {
...
*/
int CALL_SPEC _daveInitAdapterMPI1(daveInterface * di) {
    uc b2[] = {
    0x01,0x0D,0x02,
    };
...
*/
int CALL_SPEC _daveInitAdapterMPI3(daveInterface * di)
{
...
int CALL_SPEC _daveSendWithPrefix32(daveConnection * dc, int size) {
    uc fix[] = { 04,0x80,0x80,0x0C,0x03,0x14 };
    fix[4] = dc->connectionNumber2;		// 1/10/05 trying Andrew's patch
    fix[5] = dc->connectionNumber;		// 1/10/05 trying Andrew's patch
    memcpy(dc->msgOut, fix, sizeof(fix));
...
int CALL_SPEC _daveListReachablePartnersMPI3(daveInterface * di, char * buf) {
    uc b1[daveMaxRawLen];
    uc m1[] = { 1,7,2 };
    int res;// , len;
...
*/
int CALL_SPEC _daveConnectPLCMPI3(daveConnection * dc) {
    int res, mpi, len;
    PDU p1;
    uc b1[daveMaxRawLen];
    uc * pcha;
...
/*
    ...
    and adding DLE,ETX and bcc.
*/
int CALL_SPEC _daveSendWithPrefix31(daveConnection * dc, uc *b, int size)
{
    uc target[daveMaxRawLen];
    uc fix[] = { 04,0x80,0x80,0x0C,0x03,0x14 };
    fix[4] = dc->connectionNumber2; 		// 1/10/05 trying Andrew's patch
...
/*
    Executes part of the dialog necessary to send a message:
*/
int CALL_SPEC _daveSendDialog3(daveConnection * dc, int size)
{
    if (size > 5) {
        dc->needAckNumber = dc->messageNumber;
...
int CALL_SPEC _daveSendMessageMPI3(daveConnection * dc, PDU * p) {
    if (daveDebug & daveDebugExchange) {
...
/*
    Sends an ackknowledge message for the message number nr:
*/
int CALL_SPEC _daveSendAckMPI3(daveConnection * dc, int nr)
{
    uc m[3];
    if (daveDebug & daveDebugPacket)
        LOG3("%s sendAck for message %d \n", dc->iface->name, nr);
...
int CALL_SPEC _daveGetResponseMPI3(daveConnection *dc) {
    int res, count;
    if (daveDebug & daveDebugExchange)
...
int CALL_SPEC _daveExchangeMPI3(daveConnection * dc, PDU * p) {
    _daveSendMessageMPI3(dc, p);
    dc->AnswLen = 0;
    return _daveGetResponseMPI3(dc);
}

int CALL_SPEC _daveDisconnectPLCMPI3(daveConnection * dc)
{
    //    uc m[]={
    //        0x80
    //    };
...
*/
int CALL_SPEC _daveDisconnectAdapterMPI3(daveInterface * di) {
    //    uc m3[]={
    //        0x80
    //    };
    uc m2[] = {
...
/*
    ...
    from adapter are not as expected.
*/
int CALL_SPEC _daveDisconnectAdapterMPI(daveInterface * di) {
    int res;
    uc m2[] = {
        1,4,2
    };
...
*/
int CALL_SPEC _daveListReachablePartnersMPI(daveInterface * di, char * buf) {
    uc b1[daveMaxRawLen];
    uc m1[] = { 1,7,2 };
    int res;
...
int CALL_SPEC _daveDisconnectPLCMPI(daveConnection * dc)
{
    int res;
    uc m[] = {
        0x80
...
*/
int CALL_SPEC _daveNegPDUlengthRequest(daveConnection * dc, PDU *p) {
    uc pa[] = { 0xF0, 0 ,0, 1, 0, 1, 0, 0};
    int res;
    int CpuPduLimit;
    PDU p2;

    pa[6] = dc->maxPDUlength / 0x100;//, //3,
    pa[7] = dc->maxPDUlength % 0x100;//, //0xC0,
    //};
    p->header = dc->msgOut + dc->PDUstartO;
    _daveInitPDUheader(p, 1);
    _daveAddParam(p, pa, sizeof(pa));
...
/*
    ...
    (or reused for the same PLC ?)
*/
int CALL_SPEC _daveConnectPLCMPI2(daveConnection * dc) {
    int res;
    PDU p1;
    uc b1[daveMaxRawLen];
...
/*
    ...
    (or reused for the same PLC ?)
*/
int CALL_SPEC _daveConnectPLCMPI1(daveConnection * dc) {
    int res, len;
    PDU p1;
    uc * pcha;
...
*/
#ifdef HAVE_SELECT
int CALL_SPEC _daveReadOne(daveInterface * di, uc *b) {
    fd_set FDS;
    struct timeval t;
    FD_ZERO(&FDS);
    FD_SET(di->fd.rfd, &FDS);
...
*/
int CALL_SPEC _daveTimedRecv(daveInterface * di, uc *b, int len) {
    fd_set FDS;
    struct timeval t;
    FD_ZERO(&FDS);
#ifdef OS_WINDOWS
...
int CALL_SPEC _daveReadIBHPacket2(daveInterface * di, uc *b) {
    int res, len;
...
int CALL_SPEC _daveReadIBHPacket(daveInterface * di, uc *b) {
    int res, res2;// , len2;
    uc b2[300];
    ...
    again:
        ...
        res2 = send((SOCKET)(di->fd.wfd), IBHfollow, 15, 0);
...
*/
int CALL_SPEC _daveReadISOPacket(daveInterface * di, uc *b) {
...
int CALL_SPEC _daveSendISOPacket(daveConnection * dc, int size) {
    int res;
    size += 4;
...
int CALL_SPEC _daveGetResponseISO_TCP(daveConnection * dc) {
    int res;
...
*/
int CALL_SPEC _daveExchangeTCP(daveConnection * dc, PDU * p) {
    int res, totLen, sLen;
...
int CALL_SPEC _daveConnectPLCTCP(daveConnection * dc) {
    ...
    0xC1,2,1,0,
    0xC2,2,
    0, // b4[13] = dc->communicationType; //,
    0, // b4[14] = (dc->slot | dc->rack << 5); //, // hope I got it right this time...
    0xC0,1,0x9,
    };

    uc b4R2[] = {			// for routing
    6 + 30 + 30 + 3, // 0		// Length over all without this byte (6 byte fixed data, 30 bytes source TSAP (C1), 30 bytes dest TSAP (C2), 3 bytes TPDU size (C0))

    0xE0,		// 1 // TDPU Type CR = Connection Request (see RFC1006/ISO8073)
    0x00,0x00,	// 2, 3 // TPDU Destination Reference (unknown)
    0x00,0x01,	// 4, 5 // TPDU Source-Reference (my own reference, should not be zero)

    0x00,		// 6 // TPDU Class 0 and no Option

    0xC1,		// 7 // Parameter Source-TSAP
    28,		// 8 Length of this parameter
    1,		// 9 one block of data (???)
    0,		// 10 Length for S7-Subnet-ID
    0,		// 11 Length of PLC-Number
    2,		// 12 Length of Function/Rack/Slot
    0,0,0,0, 0,0,0,0, // 13 - 20	// empty Data
    0,0,0,0, 0,0,0,0, // 21 - 28
    0,0,0,0, 0,0,     // 29 - 34
    1,		// 35 Function (1=PG,2=OP,3=Step7Basic)
    0,		// 36 Rack (Bit 7-5) and Slot (Bit 4-0)

    0xC2,		// 37 Parameter Destination-TSAP
    28,		// 38 Length of this parameter
    1,		// 39 one block of data (???)

    6,		// 40 Length for S7-Subnet-ID
    1,		// 41 Length of PLC address
    2,		// 42 Length of Function/Rack/Slot

    0x01,0x52,     // 43, 44		// first part of S7-Subnet-ID  (look into the S7Project/Network configuration)
    0x00,0x00,     // 45, 46		// fix always 0000 (reserved for later use ?)
    0x00,0x13,     // 47, 48		// second part of S7-Subnet-ID
                // (see S7Project/Network configuration)

    0x01,	      // 49		// PLC address

    0,0,0,0,0,0,0,0, // 50 - 57	// empty
    0,0,0,0,0,0,0,   // 58 - 64

    0, // b4R2[65] = dc->communicationType; //,		// Function (1=PG,2=OP,3=Step7Basic)
    0, // b4R2[66] = (dc->slot | dc->rack << 5); //,	// Rack (Bit 7-5) and Slot (Bit 4-0) hope I got it right this time...

    0xC0,		// 67 Parameter requested TPDU-Size
    1,		// 68 Length of this parameter
    0x9,		// 69 requested TPDU-Size 8=256 Bytes, 9=512, a=1024 Bytes
    };
    ...
    PDU p1;
        ...
        b4[13] = dc->communicationType; //,
    b4[14] = (dc->slot | dc->rack << 5); //, // hope I got it right this time...

        b4R2[65] = dc->communicationType; //,		// Function (1=PG,2=OP,3=Step7Basic)
    b4R2[66] = (dc->slot | dc->rack << 5); //,	// Rack (Bit 7-5) and Slot (Bit 4-0) hope I got it right this time...
...
void CALL_SPEC _daveSendLength(daveInterface * di, int len) {
    uc c[] = { 104,0,0,104 };
    c[1] = len;
    c[2] = len;
...
void CALL_SPEC _daveSendIt(daveInterface * di, uc * b, int size) {
    int i;
    us sum = 0;
    for (i = 0; i < size; i++) {
        sum += b[i];
...
void CALL_SPEC _daveSendRequestData(daveConnection * dc, int alt) {
...
int CALL_SPEC _daveGetResponsePPI(daveConnection *dc) {
...
int CALL_SPEC _daveExchangePPI(daveConnection * dc, PDU * p1) {
    int i, res = 0, len;
...
int CALL_SPEC _daveConnectPLCPPI(daveConnection * dc) {
    PDU p;
...
*/
int CALL_SPEC daveInitAdapter(daveInterface * di) {
    return di->initAdapter(di);
}

int CALL_SPEC daveConnectPLC(daveConnection * dc) {
    return dc->iface->connectPLC(dc);
}

int CALL_SPEC daveDisconnectPLC(daveConnection * dc) {
    return dc->iface->disconnectPLC(dc);
}

int CALL_SPEC daveDisconnectAdapter(daveInterface * di) {
    return di->disconnectAdapter(di);
}

int CALL_SPEC _daveExchange(daveConnection * dc, PDU *p) {
    int res;
...
int CALL_SPEC daveSendMessage(daveConnection * dc, PDU *p) {
    return dc->iface->sendMessage(dc, p);
}

int CALL_SPEC daveListReachablePartners(daveInterface * di, char * buf) {
    return di->listReachablePartners(di, buf);
}

int CALL_SPEC daveGetResponse(daveConnection * dc) {
...
*/
int CALL_SPEC daveGetS8from(uc *b) {
    char* p = (char*)b;
    return *p;
}

int CALL_SPEC daveGetU8from(uc *b) {
    return *b;
}

int CALL_SPEC daveGetS16from(uc *b) {
    union {
        short a;
        uc b[2];
    } u;
...
int CALL_SPEC daveGetU16from(uc *b) {
    union {
        unsigned short a;
        uc b[2];
    } u;
...
int CALL_SPEC daveGetS32from(uc *b) {
    union {
        int a;
        uc b[4];
    } u;
...
unsigned int CALL_SPEC daveGetU32from(uc *b) {
    union {
        unsigned int a;
        uc b[4];
    } u;
...
float CALL_SPEC daveGetFloatfrom(uc *b) {
    union {
        float a;
        uc b[4];
    } u;
...
*/
int CALL_SPEC daveGetS8(daveConnection * dc) {
    char * p;
#ifdef DEBUG_CALLS
    LOG2("daveGetS8(dc:%p)\n", dc);
    FLUSH;
...
int CALL_SPEC daveGetU8(daveConnection * dc) {
    uc * p;
#ifdef DEBUG_CALLS
    LOG2("daveGetU8(dc:%p)\n", dc);
    FLUSH;
...
int CALL_SPEC daveGetS16(daveConnection * dc) {
    union {
        short a;
        uc b[2];
    } u;
...
int CALL_SPEC daveGetU16(daveConnection * dc) {
    union {
        unsigned short a;
        uc b[2];
    } u;
...
int CALL_SPEC daveGetS32(daveConnection * dc) {
    union {
        int a;
        uc b[4];
    } u;
...
unsigned int CALL_SPEC daveGetU32(daveConnection * dc) {
    union {
        unsigned int a;
        uc b[4];
    } u;
...
*/
int CALL_SPEC daveGetS8At(daveConnection * dc, int pos) {
    char * p = (char *)(dc->_resultPointer);
    p += pos;
    return *p;
}

int CALL_SPEC daveGetU8At(daveConnection * dc, int pos) {
    uc * p = (uc *)(dc->_resultPointer);
    p += pos;
    return *p;
}

int CALL_SPEC daveGetS16At(daveConnection * dc, int pos) {
    union {
        short a;
        uc b[2];
    } u;
...
int CALL_SPEC daveGetU16At(daveConnection * dc, int pos) {
    union {
        unsigned short a;
        uc b[2];
    } u;
...
int CALL_SPEC daveGetS32At(daveConnection * dc, int pos) {
    union {
        int a;
        uc b[4];
    } u;
...
unsigned int CALL_SPEC daveGetU32At(daveConnection * dc, int pos) {
    union {
        unsigned int a;
        uc b[4];
    } u;
...
*/
uc * CALL_SPEC davePut8(uc *b, int v) {
    *b = v & 0xff;
    b++;
    return b;
}

uc * CALL_SPEC davePut16(uc *b, int v) {
    union {
        short a;
        uc b[2];
    } u;
...
uc * CALL_SPEC davePut32(uc *b, int v) {
    union {
        int a;
        uc b[2];
    } u;
...
uc * CALL_SPEC davePutFloat(uc *b, float v) {
    union {
        float a;
        uc b[2];
    } u;
...
void CALL_SPEC davePut8At(uc *b, int pos, int v) {
    union {
        short a;
        uc b[2];
    } u;
...
void CALL_SPEC davePut16At(uc *b, int pos, int v) {
    union {
        short a;
        uc b[2];
    } u;
...
void CALL_SPEC davePut32At(uc *b, int pos, int v) {
    union {
        int a;
        uc b[2];
    } u;
...
void CALL_SPEC davePutFloatAt(uc *b, int pos, float v) {
    union {
        float a;
        uc b[2];
    } u;
...
void CALL_SPEC _daveConstructReadResponse(PDU * p) {
    uc pa[] = { 4,1 };
    uc da[] = { 0xFF,4,0,0 };
...
void CALL_SPEC _daveConstructBadReadResponse(PDU * p) {
    uc pa[] = { 4,1 };
    uc da[] = { 0x0A,0,0,0 };
    _daveInitPDUheader(p, 3);
    _daveAddParam(p, pa, sizeof(pa));
...
void CALL_SPEC _daveConstructWriteResponse(PDU * p) {
    uc pa[] = { 5,1 };
    uc da[] = { 0xFF };
    _daveInitPDUheader(p, 3);
    _daveAddParam(p, pa, sizeof(pa));
...
void CALL_SPEC _daveHandleRead(PDU * p1, PDU * p2) {
    int result;
...
void CALL_SPEC _daveHandleWrite(PDU * p1, PDU * p2) {
...
int CALL_SPEC _daveWriteIBH(daveInterface * di, uc * buffer, int len) { //cs: let it be uc
    int res;
    if (daveDebug & daveDebugByte) {
...
int CALL_SPEC _davePackPDU(daveConnection * dc, PDU *p) {
    IBHpacket * ibhp;
...
void CALL_SPEC _daveSendMPIAck_IBH(daveConnection*dc) {
...
*/
void CALL_SPEC _daveSendIBHNetAck(daveConnection * dc) {
    IBHpacket * p;
    uc ack[13];
...
*/
int CALL_SPEC __daveAnalyze(daveConnection * dc) {
    int haveResp;
    IBHpacket * p, *p2;
    MPIheader * pm;
    MPIheader2 * m2;
...
int CALL_SPEC _daveExchangeIBH(daveConnection * dc, PDU * p) {
...
int CALL_SPEC _daveSendMessageMPI_IBH(daveConnection * dc, PDU * p) {
    int res;
    _davePackPDU(dc, p);
...
int CALL_SPEC _daveGetResponseMPI_IBH(daveConnection * dc) {
    int res, count, pt;
    count = 0;
    pt = 0;
    do {
...
*/
int CALL_SPEC _daveInitStepIBH(daveInterface * iface, uc * chal, int cl, us* resp, int rl, uc*b) {
    int res, res2, a = 0;
    if (daveDebug & daveDebugConnect)
...
*/
int CALL_SPEC _daveConnectPLC_IBH(daveConnection*dc) {
    int a, retries;
    PDU p1;
    uc b[daveMaxRawLen];
    uc * pcha;
...
*/
int CALL_SPEC _daveDisconnectPLC_IBH(daveConnection*dc) {
    uc b[daveMaxRawLen];
...
*/
int CALL_SPEC daveForceDisconnectIBH(daveInterface * di, int src, int dest, int mpi) {
    uc b[daveMaxRawLen];
    chal31[8] = src;
    chal31[9] = dest;
    chal31[10] = mpi;
...
/*
    ...
    Returns 0 for success and a negative number on errors.
*/
int CALL_SPEC daveResetIBH(daveInterface * di) {
    uc chalReset[] = {
...
void CALL_SPEC _daveSendMPIAck2(daveConnection *dc) {
    IBHpacket * p;
    uc c;
    uc ack[18];
    memcpy(ack, dc->msgIn, sizeof(ack));
...
int CALL_SPEC _davePackPDU_PPI(daveConnection * dc, PDU *p) {
    IBHpacket * ibhp;
...
/*
    send a network level ackknowledge
*/
void CALL_SPEC _daveSendIBHNetAckPPI(daveConnection * dc) {
...
int CALL_SPEC _daveListReachablePartnersMPI_IBH(daveInterface * di, char * buf) {
    int a, i;
...
/*
    packet analysis. mixes all levels.
*/
int CALL_SPEC __daveAnalyzePPI(daveConnection * dc, uc sa) {
    IBHpacket* p;
...
int CALL_SPEC _daveExchangePPI_IBH(daveConnection * dc, PDU * p) {
    int res, count, pt;
...
int CALL_SPEC _daveGetResponsePPI_IBH(daveConnection * dc) {
    int res, count, pt;
    count = 0;
    do {
        _daveSendIBHNetAckPPI(dc);
...
*/
int CALL_SPEC BuildAndSendPDU(daveConnection * dc, PDU*p2, uc *pa, int psize, uc *ud, int usize,
    uc *ud2, int usize2) {
    int res;
    PDU p, *p3;
    uc * dn;
...
int CALL_SPEC daveForce200(daveConnection * dc, int area, int start, int val) {
    int res;
    PDU p2;
    //    uc pa[]={0,1,18,4,17,67,2,0};
    //    uc da[]={'0','0'};
...
daveResultSet * CALL_SPEC daveNewResultSet() {
...
void CALL_SPEC daveFree(void * dc) {
...
int CALL_SPEC daveGetProgramBlock(daveConnection * dc, int blockType, int number, char* buffer, int * length) {
    int res, uploadID, more;// , len;
...
int CALL_SPEC daveDeleteProgramBlock(daveConnection*dc, int blockType, int number) {
    int res;
    PDU p, p2;
    uc paDelete[] = {
...
int CALL_SPEC daveReadPLCTime(daveConnection * dc) {
    int res, len;
    PDU p2;
...
int CALL_SPEC daveSetPLCTime(daveConnection * dc, uc * ts) {
    int res, len;
    PDU p2;
...
uc CALL_SPEC daveToBCD(uc i) {
    return 16 * (i / 10) + (i % 10);
}

uc CALL_SPEC daveFromBCD(uc i) {
    return 10 * (i / 16) + (i % 16);
}

int CALL_SPEC daveSetPLCTimeToSystime(daveConnection * dc) {
    int res, len;
    PDU p2;
    uc pa[] = { 0,1,18,4,17,'G',2,0 };
    uc ts[] = {
...
*/
int CALL_SPEC daveReadS5Bytes(daveConnection * dc, uc area, uc BlockN, int offset, int count)
{
...
*/
void CALL_SPEC _daveSendDLEACK(daveInterface * di)	// serial interface
{
...
*/
int CALL_SPEC _daveSendWithDLEDup(daveInterface * di, // serial interface
    uc *b, 		    // a buffer containing the message
    int size		    // the size of the string
)
{
...
*/
int CALL_SPEC _daveReqTrans(daveConnection * dc, uc trN)
{
    uc b1[3];
    int res;
    if (daveDebug & daveDebugExchange)
...
*/
int CALL_SPEC _daveEndTrans(daveConnection * dc)
{
    int res;
    uc b1[3];
    if (daveDebug & daveDebugExchange)
...
*/
int CALL_SPEC _daveDLEDeDup(daveConnection * dc, uc* rawBuf, int rawLen) {
    int j = 0, k;
...
int CALL_SPEC _daveExchangeAS511(daveConnection * dc, uc * b, int len, int maxlen, int trN) {
    int res, i;
    uc b1[3];
...
*/
int CALL_SPEC _daveReadS5BlockAddress(daveConnection * dc, uc area, uc BlockN, daveS5AreaInfo * ai)
{
...
int CALL_SPEC _daveIsS5BlockArea(uc area)
{
    if (
...
int CALL_SPEC _daveIsS5DBlockArea(uc area)
{
    if (area != daveDB) {
...
*/
int CALL_SPEC _daveFakeExchangeAS511(daveConnection * dc, PDU *p) {
...
int CALL_SPEC _daveConnectPLCAS511(daveConnection * dc) {
    int res;
...
int CALL_SPEC _daveDisconnectPLCAS511(daveConnection * dc) {
    free(dc->cache);
    dc->cache = 0;
    return 0;
}
...
*/
int CALL_SPEC daveWriteS5Bytes(daveConnection * dc, uc area, uc BlockN, int offset, int count, void * buf)
{
    int res, datastart;
    daveS5AreaInfo ai;
    uc b1[daveMaxRawLen];
...
int CALL_SPEC daveStopS5(daveConnection * dc) {
    uc b1[] = { 0x88,0x04 };	// I don't know what this mean
    return daveWriteBytes(dc, daveSysDataS5, 0, 0x0c, 2, b1);
}

int CALL_SPEC daveStartS5(daveConnection * dc) {
    uc b1[] = { 0x68,0x00 };	// I don't know what this mean
    return daveWriteBytes(dc, daveSysDataS5, 0, 0x0c, 2, b1);
}

int CALL_SPEC daveGetS5ProgramBlock(daveConnection * dc, int blockType, int number, char* buffer, int * length) {
    //    int totlen,res;
    //    *length=totlen;
...
*/
int CALL_SPEC _daveSCP_send(int fd, uc * reqBlock) {
...
int CALL_SPEC _daveConnectPLCS7online(daveConnection * dc) {
...
int CALL_SPEC _daveSendMessageS7online(daveConnection *dc, PDU *p) {
    int a;
    int datalen;
...
int CALL_SPEC _daveDisconnectPLCS7online(daveConnection * dc) {
    int co, er, a;
...
int CALL_SPEC _daveGetResponseS7online(daveConnection *dc) {
    int a;
...
int CALL_SPEC _daveExchangeS7online(daveConnection * dc, PDU * p) {
    int res;
...
int CALL_SPEC _daveListReachablePartnersS7online(daveInterface * di, char * buf) {
    int a;
    S7OexchangeBlock reqBlock;
    uc b1[sizeof(S7OexchangeBlock)];
    S7OexchangeBlock* fdr;
...
/*
int CALL_SPEC _daveDisconnectAdapterS7online(daveInterface * di) {
    int res;
...
#ifndef NET
int CALL_SPEC _daveReadMPINLpro(daveInterface * di, uc *b) {
    int res = 0, state = 0, nr_read;
    uc bcc = 0;
...
int CALL_SPEC _daveInitAdapterNLpro(daveInterface * di)  /* serial interface */
{
    uc b3[] = {
...
void CALL_SPEC _daveSendSingleNLpro(daveInterface * di,	/* serial interface */
    uc c  			/* chracter to be send */
)
{
    uc c3[3];
...
/*
    ...
    and adding DLE,ETX and bcc.
*/
int CALL_SPEC _daveSendWithCRCNLpro(daveInterface * di, /* serial interface */
    uc *b, 		 /* a buffer containing the message */
    int size		 /* the size of the string */
)
{
...
#endif
#ifdef OS_WINDOWS
    send((SOCKET)(di->fd.wfd), target, targetSize, 0);
#endif
...
/*
    Send a string of init data to the MPI adapter.
*/
int CALL_SPEC _daveInitStepNLpro(daveInterface * di, int nr, uc *fix, int len, char * caller, uc * buffer) {
    uc res[500];
    int i;
    if (daveDebug & daveDebugInitAdapter)
        LOG4("%s %s step %d.\n", di->name, caller, nr);
...
/*
    ...
    (or reused for the same PLC ?)
*/
int CALL_SPEC _daveConnectPLCNLpro(daveConnection * dc) {
    int res, len;
    PDU p1;
    uc * pcha;
...
/*
    Executes part of the dialog necessary to send a message:
*/
int CALL_SPEC _daveSendDialogNLpro(daveConnection * dc, int size)
{
    if (size > 5) {
        dc->needAckNumber = dc->messageNumber;
        dc->msgOut[dc->iface->ackPos + 1] = _daveIncMessageNumber(dc);
...
/*
    Sends a message and gets ackknowledge:
*/
int CALL_SPEC _daveSendMessageNLpro(daveConnection * dc, PDU * p) {
    if (daveDebug & daveDebugExchange) {
...
int CALL_SPEC _daveExchangeNLpro(daveConnection * dc, PDU * p) {
    _daveSendMessageNLpro(dc, p);
    dc->AnswLen = 0;
    return _daveGetResponseNLpro(dc);
}

int CALL_SPEC _daveGetResponseNLpro(daveConnection *dc) {
    int res;
    if (daveDebug & daveDebugExchange) {
...
int CALL_SPEC _daveSendWithPrefixNLpro(daveConnection * dc, uc *b, int size)
{
    uc target[daveMaxRawLen];
    //    uc fix[]= {04,0x80,0x80,0x0C,0x03,0x14};
...
int CALL_SPEC _daveSendWithPrefix2NLpro(daveConnection * dc, int size)
{
    //    uc fix[]= {04,0x80,0x80,0x0C,0x03,0x14};
...
int CALL_SPEC _daveDisconnectPLCNLpro(daveConnection * dc)
{
    int res;
    uc m[] = {
        0x80
...
/*
    ...
    from adapter are not as expected.
*/
int CALL_SPEC _daveDisconnectAdapterNLpro(daveInterface * di) {
    int res;
    uc m2[] = {
        1,4,2
    };
...
/*
*/
int CALL_SPEC _daveListReachablePartnersNLpro(daveInterface * di, char * buf) {
    uc b1[daveMaxRawLen];
    uc m1[] = { 1,7,2 };
    int res;
